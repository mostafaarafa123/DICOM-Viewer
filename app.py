...
import numpy as np  # Library to handle arrays and numeric operations
...
def process_image(img, brightness, contrast, filter_type, threshold, zoom):
    ...
    if zoom != 1.0:
        ...
        new_w, new_h = int(w / zoom), int(h / zoom)  # Calculate the new width and height for zoomed cropping
        x0, x1 = max(0, center_x - new_w // 2), min(w, center_x + new_w // 2)  # Crop boundaries X-axis
        y0, y1 = max(0, center_y - new_h // 2), min(h, center_y + new_h // 2)  # Crop boundaries Y-axis
        img_processed = img_processed[y0:y1, x0:x1]  # Crop
        img_processed = cv2.resize(img_processed, (w, h))  # Resize back to original size

    return img_processed

# Function to create animated GIF from DICOM slices
...
# Main code runs only if files are uploaded
if uploaded_files:
    ...
    for tag in ['PatientName', 'PatientID', 'StudyDate', 'Modality', 'SliceThickness', 'PixelSpacing']:
        ...
        metadata[tag] = safe_str(val)  # Convert metadata value safely to string
